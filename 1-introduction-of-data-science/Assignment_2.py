...
def answer_two():
    print((df['Gold'] - df['Gold.1']).idxmax())
...
def answer_three():
    ...
    df_gold_diff = ((abs(df_gold['Gold'] - df_gold['Gold.1'])) / df_gold['Gold.2'])
    # print(df_gold_diff)
    gold_diff_max = df_gold_diff.idxmax()
    print(gold_diff_max)
    return gold_diff_max
...
# counts for 3 points, silver medals (`Silver.2`) for 2 points, and bronze medals (`Bronze.2`) for 1 point.
...
def answer_five():
    # print(census_df.groupby(['STNAME']).sum()['COUNTY'])
...
def answer_six():
    ...
    ans= county_df.groupby('STNAME')['CENSUS2010POP'].apply(lambda x: x.nlargest(3).sum()).nlargest(3).index.values.tolist()
    return ans
...
def answer_eight():
    ...
    start_washington_df = region_df[region_df['CTYNAME'].str.startswith('Washington')]
    population_df = start_washington_df[(start_washington_df['POPESTIMATE2015'] >start_washington_df['POPESTIMATE2014'])]
    ans = population_df[['STNAME', 'CTYNAME']]
    return ans
